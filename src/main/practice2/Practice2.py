...
def scraiping(driver: webdriver.Chrome, target_user_name: str):
    '''ホーム画面操作'''
    # 操作するユーザのトップ画面に遷移 # TODO (コメントを外して~~~を正しく記載)
    # driver.~~~("https://www.instagram.com/" + target_user_name)
    ...
    # TODO class名「_aagv」が投稿ごとのdivタグのclass属性に割り振られているため、それらをすべて取得しに行く
    home_div_tags = None
    ...
    for home_div_tag in home_div_tags:
        # 投稿の中のimgタグを取得
        # TODO Seleniumには取得したelementに対しても検索をすることができる。よって、取得しているdivタグの中にある「img」タグを取得しに行く
        home_img_tag = None

        # imgタグ内のsrc属性の値(画像リンク)を抽出
        # TODO 上で取得したimgタグの中にある「src」属性を同じ要領で取得する(属性を取るには「element.get_attribute("属性名")」)
        src = None
